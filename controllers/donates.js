  ...
  upload.single('image')(req, res, async error => {
    ...
    if (error instanceof multer.MulterError) {
      ...
      try {
        ...
        const result = await donates.create({
...
export const edit = async (req, res) => {
  ...
  try {
    let result = await donates.findById(req.params.id)
    if (result === null) {
      res.status(404).send({ success: false, message: '找不到資料' })
    } else if (!req.session.member.right) {
      res.status(403).send({ success: false, message: '沒有權限' })
    } else {
      result = await donates.findByIdAndUpdate(req.params.id, req.body, { new: true })
      res.status(200).send({ success: true, message: '', result })
    }
  } catch (error) {
    console.log(error)
...
export const clear = async (req, res) => {
  ...
  try {
    const result = await donates.findByIdAndDelete(req.params.id)
    if (result === null) {
      res.status(404).send({ success: false, message: '找不到資料' })
    } else {
...
export const donation = async (req, res) => {
  try {
    const result = await donates.findById(req.params.id)
    res.status(200).send({ success: true, message: '', result })
  } catch (error) {
    console.log(error)
    res.status(500).send({ success: false, message: '伺服器錯誤' })
